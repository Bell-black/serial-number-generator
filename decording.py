...
class SerialApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        self.merge_checkbox = ctk.CTkCheckBox(container, text="SVG 한 장으로 저장")
        self.merge_checkbox.pack(anchor="w", pady=(5, 10))
        ...
        ctk.CTkLabel(decode_frame, text="시리얼 넘버 해석:").pack(side="left")
        self.decode_entry = ctk.CTkEntry(decode_frame, width=200)
        self.decode_entry.pack(side="left", padx=(5, 5))
        decode_btn = ctk.CTkButton(decode_frame, text="해석", command=self.decode_serial_ui)
    ...
    def generate_serials(self):
        ...
        merge_svgs_checked = self.merge_checkbox.get()
